...
static unsigned long a_crc32_table[256u] = {
...
unsigned long cv_crc32_convert(struct cv_array const * p_buffer) {
    ...
    cv_crc32_cleanup( &o_crc32 );
    return i_crc32_value;
} /* cv_crc32() */
...
void cv_crc32_init( struct cv_crc32 * p_this) {
    p_this->a_private[0u] = 0xFFFFFFFFul;
    p_this->a_private[1u] = 0ul;
} /* cv_crc32_init() */
...
{
    unsigned char const * p_data_iterator = p_buffer->o_min.pc_uchar;
    unsigned long i_crc32_value = p_this->a_private[0u];
    while ( p_data_iterator < p_buffer->o_max.pc_uchar) {
        i_crc32_value = ( ( (i_crc32_value << 8u) ^ a_crc32_table[
                ( (i_crc32_value >> 24u) ^ (*p_data_iterator)) & 0xFFu]) &
            0xFFFFFFFFul);
        p_data_iterator ++;
    }
    p_this->a_private[0u] = i_crc32_value;
} /* cv_crc32_write() */
...
unsigned long cv_crc32_result( struct cv_crc32 const * p_this) {
    return p_this->a_private[0u];
} /* cv_crc32_result() */

/* end-of-file: cv_crc32.c */
