...
cv_uptr cv_utf8_encoder_produce(cv_utf8_encoder * p_this,
    ...
    if (0x7ful >= i_input) {
        ...
        p_this->a_accum[0u] = i_input & 0x3f;
        p_this->i_count = 2;
    } else if (0xfffful >= i_input) {
        unsigned long const l_temp1 = ((i_input >> 12u) & 0x0ful) | 0xe0ul;
        unsigned long const l_temp2 = (i_input >> 6u) & 0x3ful;
        p_this->a_accum[2u] = l_temp1 & 0xff;
        p_this->a_accum[1u] = l_temp2 & 0xff;
        p_this->a_accum[0u] = i_input & 0x3f;
        p_this->i_count = 3;
    } else if (0x10fffful >= i_input) {
        unsigned long const l_temp1 = ((i_input >> 18u) & 0x0ful) | 0xe0ul;
        unsigned long const l_temp2 = (i_input >> 12u) & 0x3ful;
        unsigned long const l_temp3 = (i_input >> 6u) & 0x3ful;
