...
cv_heap_node * cv_heap_small_lookup( cv_heap_small * p_this, cv_uptr i_len) {
    ...
    cv_debug_assert_(i_len > 0, cv_debug_code_invalid_length);
    {
        ...
        cv_uptr const i_aligned_len =
            cv_sizeof_align(i_len, cv_heap_small_align_);
        /* Locate pool */
        cv_uptr const i_pool_index = (i_aligned_len - 1) / cv_heap_small_align_;
        if (i_pool_index < cv_heap_small_count_) {
...
void cv_heap_small_free( cv_heap_small * p_this, cv_heap_node * p_heap_node) {
    ...
    {
        cv_uptr const i_len = cv_array_len(&p_heap_node->o_payload);
        cv_uptr const i_pool_index = (i_len - 1) / cv_heap_small_align_;
        if (i_pool_index < cv_heap_small_count_) {
            cv_heap_pool * const p_heap_pool =
                p_this->a_pool + i_pool_index;
